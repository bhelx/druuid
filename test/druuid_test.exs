defmodule DruuidTest do
  ...
  test "ids generated at the same time should still be unique" do
    ...
    id = Druuid.gen_from_values(0, :rand.uniform(), timestamp)
    assert(id > 0)
    assert(id != Druuid.gen_from_values(0, :rand.uniform(), timestamp))
  end
  ...
  test "ids should be sortable by time generated" do
    ...
    id = Druuid.gen_from_values(0, :rand.uniform(), timestamp)
    assert(id > 0)
    assert(id < Druuid.gen_from_values(0, :rand.uniform(), timestamp + 1))
  end
